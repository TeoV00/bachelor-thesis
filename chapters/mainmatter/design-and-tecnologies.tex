...
%TODO: il discorso non inzia bene
...
%TODO: finre e mostrare scherma pattern MVI
\begin{figure}
    \centering
...
\end{figure}
% MVI pattern utilizzato per le schermate post scansione qr code dove è presente un FutureBuilder e che quinid il dominio ritorna un dato Future che presenta degli stati, in base allo stato del dato viene mostrata una UI apposta. Il punto focale di questo pattern è che la UI rimane reattiva e il dominio aggiorna il suo stato mentre aggiorna o legge il dominio es. database. o sservizio online. Futurebuilder per la UI reattiva in base allo stato dell'intent (che in nel caso di tesi e tirocinio era un metodo specifico che ritornava un dato di tipo Future e quindi con diversi stati)

%il pattern DAO invece è stato itilizzato per astrarre e incapsulare l'accesso al db e l'implementaizome delle operazioni, infatti mi è del tutto ignoto che cosa avviene dietro al metodo che chiamo per aggiungere un albero ai progresi utente nel db. e vale lo stesso con firebase.

% pattern repository, dove la classe data manager appunto è il repository. Al suo interno ai diversi riferimenti a oggetti del dominio (che utilizzano il design pattern DAO, data access object). DAO funziona come un livello di mappatura/accesso ai dati, nascondendo brutte query. Tuttavia, un repository è un livello tra i domini e i livelli di accesso ai dati, che nasconde la complessità della raccolta dei dati e della preparazione di un oggetto di dominio
...
Il pannello dei dettagli utente si trova in alto a destra nella homepage e mostra il nickname, i progressi e il livello raggiunti dall'utente selezionato. Il design principale nelle tre versioni rimane invariato se non per la decorazione che può essere: a mensola (figura \ref{fig:shelfDetail}), a cordino (\ref{fig:cordDetail}) o tondeggiante (figura\ref{fig:roundDetail}).
\begin{figure}
    \centering
...
%TODO: elencare o parlare deiplugin utilizzati nello svilupppo del totem o della prte di app che tratta il caricamento dati (es. scansione qrcode totem)
Si elencano i 
