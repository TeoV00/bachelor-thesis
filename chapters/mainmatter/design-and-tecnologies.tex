...
L'app è stata progettata per essere in grado di funzionare offline dopo una prima sincronizzazione necessaria.
...
\begin{figure} [h]
    \centering
    \includegraphics[width=0.7\textwidth]{img/totem/arch-totem-app-dati.png}
    \caption{Schema della architettura e interazione fra app e totem}
    \label{fig:communication-schema}
\end{figure}


Per quanto riguarda l'architettura interna del software, sia per il totem che per l'app mobile, viene utilizzata una forma del pattern architetturale MVI (\textit{Model-View-Intent}).
Il Model rappresenta lo stato dell'applicazione, la View  e l'Intent è un'azione o un evento che indica un'interazione dell'utente o un cambiamento di stato.
\begin{itemize}
    %TODO: speigare meglio i tre compnenti dell'architettura MVI, come se lo dovessi psiegare a luchino
    \item La \textbf{View} è responsabile dell'interfaccia grafica e della visualizzazione dei dati ricevuti dal Model. Ad ogni evento, generato dall'interazione dell'utente, la View reagisce e comunica con il Model attraverso gli Intents.
    \item Il \textbf{Model} rappresenta lo stato dell'applicazione e contiene dati e informazioni immutabili. Il Model viene aggiornato solo generando un nuovo oggetto di stato. Nel caso dell'applicativo del totem ci si riferisce alle strutture dati immutabili che vengono restituite dal DataManager alla View e non al DataManager stesso.
    \item Gli \textbf{Intent} sono le azioni o gli eventi generati dalla vista per interagire con il modello. Gli intenti sono inviati al modello per richiedere modifiche di stato o azioni specifiche come ad esempio il caricamento dei dati sul database nel cloud.
\end{itemize}
La \textit{View} viene aggiornata dal DataManager (\textit{Model}) attraverso un flusso unidirezionale contenente lo stato della richiesta dell'utente (\textit{Intent}) che mantiene inoltre i dati del modello aggiornati da visualizzare.
...
Nel caso del totem si ha solo \texttt{FirebaseProvider} mentre nell'app mobile sono presenti anche \texttt{DatabaseProvider} e \texttt{PreferenciesProvider} (figura \ref{fig:repositorySchemaAPP}) che forniscono l'accesso ai dati persistenti memorizzati sullo smartphone.
\begin{figure}
    \centering
    \includegraphics[width=0.7\textwidth]{img/repository.png}
    \caption{Componente repository \cite{repositoryComponent}}
    \label{fig:repository}
\end{figure}

Il repository comunica con i provider attraverso un'interfaccia DAO (\textit{Data Access Object}) specifica per ciascuno di questi e permette di definire le operazioni eseguibili nascondendo l'aspetto implementativo.
...
Figma permette di testare direttamente sugli smartphone, tablet e computer i prototipi sviluppati, utilizzare plugin o creare un progetto da un template. In assenza di connessione è possibile utilizzare la maggior parte delle funzionalità, escluse la collaborazione con colleghi e l'utilizzo di alcuni plugin.
...
Flutter porta il vantaggio di sviluppare applicazioni per piattaforme e dispositivi diversi a partire da un unico codice sorgente scritto in linguaggio Dart. Esistono molteplici librerie che permettono di sfruttare al meglio l'architettura dei dispositivi in cui dovrà essere installata l'applicazione, e nel caso di esigenze particolari, è possibile innestare codice nativo (es. Swift per ios o Java per Android) all'interno del codice scritto in Dart.
%TODO: parlare dell'hot restart e degli strumenti di debug
...
%TODO: aggiungi i vantaggi e savntaggi rispetto al nativo, perchè proprio flutter anche per il totem e quali sono le feat chiave di dart
